    ...
    FVector& OutLocalPosition, FString& OutError) const
{
    ...
    FAnimExtractContext ExtractionContext(static_cast<double>(SequenceTime), false, FDeltaTimeRecord(), false);
    ExtractionContext.InterpolationOverride = TOptional<EAnimInterpolationType>();

    for (const int32 ChainBoneIndex : BoneChain)
    {
        FTransform BoneLocal;
        Sequence->GetBoneTransform(BoneLocal, FSkeletonPoseBoneIndex(ChainBoneIndex), ExtractionContext, false);
